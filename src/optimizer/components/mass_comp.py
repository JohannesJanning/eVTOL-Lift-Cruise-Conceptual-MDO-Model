...
class MassComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('m_motor', val=0.0)
        self.add_output('m_rotor', val=0.0)
        self.add_output('m_fuselage', val=0.0)
        self.add_output('m_gear', val=0.0)
        self.add_output('m_interior', val=0.0)
        self.add_output('m_wing', val=0.0)
        self.add_output('m_empty_fraction', val=0.0)
        self.add_output('m_battery_fraction', val=0.0)
        self.add_output('m_payload_fraction', val=0.0)
    ...
    def compute(self, inputs, outputs):
        ...
        # mass fractions relative to the estimated MTOM
        m_empty_fraction = m_empty_val / MTOM_est
        m_battery_fraction = m_batt / MTOM_est
        m_payload_fraction = p.m_pay / MTOM_est

        if not np.isfinite([m_batt, m_wing, m_motor, m_rotor, m_fuselage, m_gear, m_interior, m_system_val, m_empty_val, MTOM_est]).all():
            raise FloatingPointError(
                'Non-finite value in MassComp.compute: '
                f'inputs=(E_total_req={E_total}, MTOM={MTOM_guess}, rho_bat={rho_bat}, b={b}, c={c}, '
                f'V_cruise={V_cruise}, r_cruise={r_cr}, r_hover={r_hv}, P_hover={P_hover}, '
                f'P_climb={P_climb}, P_cruise={P_cruise}) '
                f'-> m_batt={m_batt}, m_wing={m_wing}, m_motor={m_motor}, m_rotor={m_rotor}, '
                f'm_fuselage={m_fuselage}, m_gear={m_gear}, m_interior={m_interior}, '
                f'm_system={m_system_val}, m_empty={m_empty_val}, MTOM_est={MTOM_est}'
            )
        ...
        outputs['m_motor'] = m_motor
        outputs['m_rotor'] = m_rotor
        outputs['m_fuselage'] = m_fuselage
        outputs['m_gear'] = m_gear
        outputs['m_interior'] = m_interior
        outputs['m_wing'] = m_wing
        outputs['m_empty_fraction'] = m_empty_fraction
        outputs['m_battery_fraction'] = m_battery_fraction
        outputs['m_payload_fraction'] = m_payload_fraction
    ...
    def compute_partials(self, inputs, partials):
        ...
        out_names = ['m_battery', 'm_empty', 'm_system', 'MTOM_est', 'm_motor', 'm_rotor', 'm_fuselage', 'm_gear', 'm_interior', 'm_wing', 'm_empty_fraction', 'm_battery_fraction', 'm_payload_fraction']
        ...
        def fun(x):
            ...
            m_battery_fraction = m_batt / MTOM_est
            m_payload_fraction = p.m_pay / MTOM_est
            return jnp.array([m_batt, m_empty_val, m_system_val, MTOM_est, m_motor, m_rotor, m_fuselage, m_gear, m_interior, m_wing, m_empty_fraction, m_battery_fraction, m_payload_fraction])
