...
def tonal_noise_hover(T_req_prop_hover, Power_req_hover, R_prop_hover, rho, n_prop_vert, n_blade_vert, params):
    ...
    Q = Power_req_hover / n_rot_rad # Q = Power_req_hover / n_prop_vert / n_rot_rad
...
def hover_rotor_rpm(T_req_prop_hover, R_prop_hover, rho, C_T):
    ...
    thrust_safe = jnp.maximum(T_req_prop_hover, 1e-6)
    radius_safe = jnp.maximum(R_prop_hover, 1e-3)
    n_rot_s = jnp.sqrt(thrust_safe / (C_T * rho * (2.0 * radius_safe) ** 4))
    return n_rot_s * 60.0
