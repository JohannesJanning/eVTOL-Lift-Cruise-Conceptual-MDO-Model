...
analysis_values = {
    ...
    'V_cruise': 46.709,
    'V_climb': 42.2832,
    'rho_bat': float(parameters.rho_bat),
    'c_charge': 1.0,
}
...
def value(name):
    """Return a scalar OpenMDAO value for display, falling back to model
    parameters, then to manually computed values (COMPUTED_VALUES)."""
    try:
    ...
    return COMPUTED_VALUES.get(name)
...
def print_constraints(constraints):
    """Print constraint value/bound/residual/status, mirroring the
    constraints defined in run_optimize_evtol.py. residual >= 0 means the
...
print(f'Analysis time (s): {elapsed:.3f}')

input_units = {
    'b': 'm', 'c': 'm', 'r_cruise': 'm', 'r_hover': 'm',
    'V_cruise': 'm/s', 'V_climb': 'm/s', 'rho_bat': 'Wh/kg', 'c_charge': '1/h',
}
print_section('INPUT VALUES', [
    (name, name, input_units.get(name, '-'), 1.0) for name in analysis_values
])
